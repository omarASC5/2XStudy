...
if os.path.exists("./app/public/saves/" + video_id):
    with open("./app/public/saves/" + video_id + "/meta_data.json") as f:
        print("DONE {} {}".format(
            video_id,
            f.read()
        ))
...
try:
    ...
    if os.path.exists("./app/public/saves/" + video_id):
        shutil.rmtree("./app/public/saves/" + video_id)
...
make_if_not_exist("./app/public/saves/" + video_id + "/video_snippets")
make_if_not_exist("./app/public/saves/" + video_id + "/audio_snippets")
...
temp_vid = "./app/public/saves/" + video_id + "/temp.mp4"
...
video.run_process(video.FFMPEG_DIR, ["-ss", "00:00:05", "-i", temp_vid, "-vframes", "1", "-q:v", "2",
    "./app/public/saves/" + video_id + "/thumb.jpg"])
# os.remove(temp_vid)
...
with open("./app/public/saves/" + video_id + "/index.html", "w", encoding='utf-8') as f:
    f.write(template)
